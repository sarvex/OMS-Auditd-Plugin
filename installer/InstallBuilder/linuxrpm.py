...
class LinuxRPMFile:
    def __init__(self, intermediateDir, targetDir, stagingDir, variables, sections):
        ...
        self.specfileName = f"{self.intermediateDir}/rpm.spec"
    ...
    def CreateRPMDirectiveFile(self):
        ...
        with open(os.path.join(os.path.expanduser('~'), '.rpmmacros'), 'w') as rpmfile:
    ...
    def GenerateSpecFile(self):
        with open(f"{self.intermediateDir}/rpm.spec", 'w') as specfile:
            ...
            for d in self.sections["Directories"]:
                if d.type != "sysdir":
                    specfile.write(
                        f'%defattr({str(d.permissions)},{d.owner},{d.group},{str(d.permissions)}'
                        + ")\n"
                    )
                    specfile.write(f'%dir {d.stagedLocation}' + '\n')

            for f in self.sections["Files"]:
                specfile.write(f'%defattr({str(f.permissions)},{f.owner},{f.group}' + ")\n")
                if f.type == "conffile":
                    specfile.write("%config ")
                specfile.write(f.stagedLocation + '\n')

            for l in self.sections["Links"]:
                specfile.write(f'%defattr({str(l.permissions)},{l.owner},{l.group}' + ")\n")
    ...
    def StageAndProperlyNameRPM(self):
        ...
        retval = os.system(
            f"mv $(find {self.intermediateDir}/RPM-packages/ -name '*.rpm') {self.targetDir}/{rpmNewFileName}"
        )
        if retval != 0:
            print("Internal error: Unable to find generated rpm")
            exit(1)

        retval = os.system(f"rm -rf {self.intermediateDir}/RPM-packages")
        if retval != 0:
    ...
    def BuildPackage(self):
        ...
        retval = os.system(
            f"rpmbuild {targetArch} --buildroot {self.stagingDir} -bb {self.specfileName}"
        )
        if retval != 0:
